		...
		$("#sjs")
				.change(
						function() {
							...
							location.href = "menuServlet?op=findAllMenu&notSelectedDeskId="
									+ v;
						});
	});
</script>
				...
				<div class="choseLanguage">
					<div class="se">
						<select id="sjs" class="selectLanguage"
							...
							<option value=" "></option>
							<c:forEach items="${allNotSelectedDesk}" var="notSelectedDesk">
								<option class="optionLangeuage" style="font-size: 40px;"
									value="${notSelectedDesk.id }">${notSelectedDesk.id }号餐桌</option>
							</c:forEach>
						</select>
					</div>
				</div>
